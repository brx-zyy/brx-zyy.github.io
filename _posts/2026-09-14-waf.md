...
A **WAF** is a security layer placed between the client and a web application. It inspects HTTP requests and can block requests that match malicious patterns.
...
### Common attacks a WAF tries to detect
...
A WAF can use rules/signatures to identify known attack patterns:
...
When testing a WAF, don't assume:

> "The WAF blocked my request, so the application is secure."

A WAF is only a **defense layer**. During authorized testing, you want to determine:

```text
Is the WAF detecting the attack?
        │
        ▼
Is the request actually blocked?
        │
        ▼
Does the backend receive it?
        │
        ▼
Is the underlying vulnerability still present?
```

> **WAF = filtering layer, not a fix for the vulnerability.**

The application should still use proper input validation, parameterized queries, output encoding, authentication, and authorization.
...
A **WAF bypass** means getting a request past the WAF that the backend still interprets as malicious.

### Common methods

* **Encoding:** URL, Unicode, HTML, or double encoding
* **Case changes:** Bypass poorly designed case sensitive rules
* **Whitespace:** Change spacing or separators
* **Syntax variations:** Use alternative syntax understood by the backend
* **Parameter pollution:** Send duplicate parameters and check how each layer handles them
* **HTTP methods:** Test GET, POST, PUT, PATCH, etc.
* **Content-Type:** Try different formats such as JSON or form data
* **Parser differences:** Look for different interpretations between the WAF, proxy, web server, and application

### Core idea

```text
WAF sees     → Safe
Backend sees → Malicious
```

The main goal is to find **normalization or parsing differences** between the WAF and backend.

> WAF bypass testing is not about random payloads. It is about understanding how each layer interprets the same request.
